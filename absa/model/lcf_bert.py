...
class LCF_BERT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        global_segments_indices = inputs[1]
        local_context_indices = inputs[2]
        aspect_indices = inputs[3]

        bert_global_out, _ = self.bert_spc(global_context_indices, token_type_ids=global_segments_indices)
        bert_global_out = self.dropout(bert_global_out)

        bert_local_out, _ = self.bert_local(local_context_indices)
        bert_local_out = self.dropout(bert_local_out)

        if self.opt.local_context_focus == 'cdm':
            masked_local_text_vec = self.feature_dynamic_mask(local_context_indices, aspect_indices)
            bert_local_out = torch.mul(bert_local_out, masked_local_text_vec)

        elif self.opt.local_context_focus == 'cdw':
            weighted_text_local_features = self.feature_dynamic_weighted(local_context_indices, aspect_indices)
